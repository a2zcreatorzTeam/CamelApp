...
class DetailsMarketingCamel extends Component {
    ...
    onCommentsClick = () => {
        ...
        let post_id = item.id;
        if (user != undefined) {
            camelapp.post("/get/comment",
                {
                    post_id: post_id,
                }
            ).then((res) => {
                this.props.navigation.navigate("Comments", { commentsForPost: res, user: user, post: item });
            })
        } else {
    ...
    sendWhatsAppMessage() {
        ...
        if (user?.user?.user?.id != this.state?.itemFromDetails?.user_id) {
            if (user.user.user != undefined) {
                ...
                console.log("this.state.itemFromDetails.user_whatsapp_status", this.state.itemFromDetails.user_whatsapp_status);

                //console.log("user", this.state.user)

                if (this.state.itemFromDetails.user_whatsapp_status == true || this.state.itemFromDetails.user_whatsapp_status == "1") {
            ...
            <ScrollView style={{ backgroundColor: "#ffff" }}>
                ...
                <View style={Styles.containerDetails}>
                    ...
                    <View style={{ textAlign: 'right' }}>
                        <Text style={[Styles.textHeadingg, { right: 0 }]}>{ArabicText.Title}</Text>
                        <TextInput
                        ...
                        <Text style={[Styles.textHeadingg, { right: 0 }]}>{ArabicText.Location}</Text>
                        <TextInput
                        ...
                        <Text style={[Styles.textHeadingg, { right: 0 }]}>{ArabicText.Price}</Text>
                        <TextInput
                        ...
                        <Text style={[Styles.textHeadingg, { right: 0 }]}>{ArabicText.Description}</Text>
                        <TextInput
                    ...
                    <View style={{
                        ...
                        <TouchableOpacity onPress={() => this.onCommentsClick()} style={{ justifyContent: "center", alignItems: "center", margin: 8 }}>
                            <Feather name="message-square" size={30} color="#CD853F" />
                            <Text style={Styles.fontDetails}>{ArabicText.comments}</Text>
                        </TouchableOpacity>

                        <TouchableOpacity onPress={() => this.sendWhatsAppMessage()} style={{ justifyContent: "center", alignItems: "center", margin: 8 }} >
...
export default connect(mapStateToProps, mapDispatchToProps)(DetailsMarketingCamel);
