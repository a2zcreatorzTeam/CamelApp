...
import {
    View, TextInput, ScrollView, Text, Image, FlatList,
    TouchableOpacity,
} from 'react-native';
    ...
    newComment() {
        ...
        if (user != undefined) {
            if (this.state.newComment != "") {
                camelapp.post("/add/news/comment",
                    {
                        user_id: user.id,
                        news_id: this.state.news_id,
    ...
    render() {
        ...
        const likeCommentHandler = (item) => {
            ...
            if (item?.user?.name !== undefined) {
                camelapp.post("/news-comment-like", {
        ...
        const Item = ({
            ...
            <Card style={{ margintop: 5, marginBottom: 5, height: 70, backgroundColor: "#f3f3f3" }}>
                ...
                <View>
                    ...
                    >
                        {item.flagForLike === true ? (<TouchableOpacity
                            onPress={() => likeCommentHandler(item)}
                            style={{}}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>

                                <AntDesign name="heart" size={16} color="#CD853F" />
                                <Text style={{ fontSize: 16, paddingLeft: 5, fontWeight: 'bold', textAlign: 'right', color: 'black' }}>{commentsCount}</Text>
                            </View>
                        </TouchableOpacity>)
                            :
                            (<TouchableOpacity
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
                        <View style={{
                            ...
                            <Text style={{ fontSize: 16, paddingRight: 5, fontWeight: 'bold', textAlign: 'right', color: 'black' }}>{name}</Text>
                        <Text style={{ fontSize: 10,  textAlign: 'right', color: 'grey' }}>{moment(date).format('YYYY-MM-DD')}</Text>
                        </View>
                        ...
                        <View style={{
                            ...
                            <Text numberOfLines={4} style={{flex:1,  fontSize: 14, paddingRight: 5, textAlign: 'right', color: 'black' }}>{comments}</Text>
                        </View>
                        <View style={Styles.user_HomeComment}>
                            <Image
                                source={{
                                    uri:
                                        "http://www.tasdeertech.com/images/profiles/" + image
                                }}
                                style={{
                                    width: 35,
                                    height: 35,
        ...
        return (
          ...
          <Text style={{color: 'black', fontWeight: 'bold',fontSize:17}}>{this.state.newsItem.user?.name}</Text>
          <Text style={{color: 'grey' ,fontSize:10}}>{this.state.newsItem?.date}</Text>
          ...
          />
                    ...
                    <Text style={{ margin: 10, fontWeight: '700', color: 'black', fontSize: 24 }}>{this.state.newsItem.title}</Text>
                    <Image source={{
                        uri: "http://www.tasdeertech.com/public/images/news/" + this.state.newsItem.image
                    }}
                        style={Styles.image}
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'center' }}>
                        <Rating
                            ...
                            imageSize={20}
                            style={{ paddingVertical: 10 }} ratingColor={"crimson"} type="custom"
                            ratingBackgroundColor={"black"}
                        />
                        {/* tintColor="white" */}
                    </View>
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: 10, marginBottom: 10, color: 'black' }}>
